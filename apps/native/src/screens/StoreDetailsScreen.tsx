...
const StoreDetailsScreen = () => {
    ...
    const fetchStoreDetails = async () => {
        try {
            const response = await apiClient.storeApi.getStoreFromId({ id: storeId });
            setStore(response.store);
        } catch (error: any) {
            Alert.alert("Error", error.message || "Failed to fetch store details");
            navigation.goBack();
        } finally {
            setLoading(false);
        }
        ...
        <ScrollView style={styles.container}>
            ...
            <View style={styles.storeContainer}>
                ...
                <View style={styles.infoContainer}>
                    ...
                    <Text style={styles.storeType}>{store.type.replace("_", " ")}</Text>

                    {store.type === "PET_STORE" && store.petStore && (
                        <View style={styles.section}>
                            <Text style={styles.sectionTitle}>Pet Categories</Text>
                            <Text style={styles.sectionText}>{store.petStore.petCategories}</Text>
                        </View>
                    )}
                    ...
                    {store.type === "VET_STORE" && store.vetStore && (
                        <View style={styles.section}>
                            ...
                            <Text style={styles.sectionText}>{store.vetStore.vetSpecialty}</Text>
                        </View>
                    )}
                    ...
                    <View style={styles.section}>
                        ...
                        <Text style={styles.sectionText}>
                            {store.avgRating ? `${store.avgRating.toFixed(1)}/5.0` : "No ratings yet"}
                        </Text>
                    </View>
                </View>
            </View>
